...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        PeerManager servent = new PeerManager(Integer.valueOf(setup[1]));
        servent.start();
        new Main().joinNetwork(br, setup[0], servent);
    }

    // Called to join the network with a defined username und servent with choosen port
    public void joinNetwork(BufferedReader br, String username, PeerManager servent) throws Exception {
        ...
        if(!input.equals("s")) for (int i = 0; i < setup.length; i++){
            ...
            try {
                servent.connect_to_peer(address[0], Integer.valueOf(address[1]));
            }
            catch(Exception e){}
        }
    ...
    public void startChat(BufferedReader br, String username, PeerManager servent){
        try {
